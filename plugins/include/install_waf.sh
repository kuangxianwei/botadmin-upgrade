Upgrade_Nginx() {
  ...
  if [ -s "nginx-${Nginx_Version}.tar.gz" ]; then
    ...
    if ! wget -c --progress=dot:giga "http://nginx.org/download/nginx-${Nginx_Version}.tar.gz"; then
      echo "You enter Nginx Version was:${Nginx_Version}"
      echo "Error! You entered a wrong version number, please check!"
      sleep 5
      exit 1
    fi
