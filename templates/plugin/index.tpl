...
<script type="text/html" id="args-html">
    <div class="layui-card" style="padding: 20px 20px 0 20px">
        ...
        <div class="layui-card-body layui-form">
            <input name="args" value="" class="layui-input" placeholder="参数一 参数二 参数三">
...
<script>
    layui.use(['index', 'main'], function () {
        ...
        table.on('tool(table-list)', function (obj) {
            let data = obj.data, argsElem = $('#args-html');
            switch (obj.event) {
                case 'install':
                    main.popup({
                        url: url + '/install',
                        ...
                        success: function (dom) {
                            if (Array.isArray(data.args) && data.args.length > 0) dom.find('input[name=args]').attr('placeholder', data.args.join(' '));
