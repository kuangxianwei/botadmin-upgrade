<script>
    ...
    class AceEditor {
        ...
        setEditorView() {
            let othis = this,
                eElem = $('.aceEditors'),
                eHeight = eElem.height(),
                autoAceHeight = setInterval(function () {
                    let pageHeight = eElem.height(),
                        aceContainerMenu = $('.ace_container_menu').height(),
                        aceContainerToolbar = $('.ace_container_toolbar').height(),
                        _height = pageHeight - ($('.menu-toggle').hasClass('active') ? 35 : 0) - aceContainerMenu - aceContainerToolbar - 42;
                    $('.ace_container_editor').height(_height);
                    if (eHeight === eElem.height()) {
                        if (othis.aceActive) othis.editor[othis.aceActive].ace.resize();
        ...
        // 事件编辑器-方法，事件绑定
        eventEditor() {
            let othis = this, icon = '<span class="icon"><i class="layui-icon layui-icon-ok"></i></span>';
            $(window).resize(function () {
                if (othis.aceActive !== undefined) othis.setEditorView()
            ...
            // 切换TAB视图
            $('.ace_container_menu').on('click', '.item', function (e) {
                var _id = $(this).attr('data-id'), _item = othis.editor[_id]
                $('.item_tab_' + _id).addClass('active').siblings().removeClass('active');
                $('#ace_editor_' + _id).addClass('active').siblings().removeClass('active');
                othis.aceActive = _id;
                othis.currentStatusBar(_id);
                othis.is_file_history(_item);
            });
            // 移上TAB按钮变化，仅文件被修改后
            $('.ace_container_menu').on('mouseover', '.item .icon-tool', function () {
            ...
            // 移出tab按钮变化，仅文件被修改后
            $('.ace_container_menu').on('mouseout', '.item .icon-tool', function () {
                var type = $(this).attr('data-file-state');
                if (type != '0') {
            ...
            // 关闭编辑视图
            $('.ace_container_menu').on('click', '.item .icon-tool', function (e) {
            ...
            // 底部状态栏功能按钮
            $('.ace_container_toolbar .pull-right span').click(function (e) {
                var _type = $(this).attr('data-type'),
                    _item = othis.editor[othis.aceActive];
                ...
                switch (_type) {
                        ...
                        othis.layer_view = layer.open({
                            ...
                            success: function (layeo, index) {
                                ...
                                $('.open_history_file').click(function () {
                                    ...
                                    othis.openHistoryEditorView({filename: _item.path, history: _history}, function () {
                                        layer.close(index);
                                        $('.ace_container_tips').show();
                                        $('.ace_container_tips .tips').html('只读文件，文件为' + _item.path + '，历史版本 [ ' + bt.format_data(new Number(_history)) + ' ]<a href="javascript:;" class="ml35 btlink" data-path="' + _item.path + '" data-history="' + _history + '">点击恢复当前历史版本</a>');
                                    });
                                });
                                $('.recovery_file_historys').click(function () {
                                    othis.event_recover_file(this);
                                });
                            }
                        });
                        break;
                    case 'tab':
                        ...
                        $('.tabsType').find(_item.softTabs ? '[data-value="nbsp"]' : '[data-value="tabs"]').addClass('active').append(icon);
                        $('.tabsSize [data-value="' + _item.tabSize + '"]').addClass('active').append(icon);
                        break;
                    case 'encoding':
                        othis.getEncodingList(_item.encoding);
            ...
            $('.menu-tabs').on('click', 'li', function (e) {
                var _val = $(this).attr('data-value'), _item = othis.editor[othis.aceActive];
            ...
            $('.menu-encoding').on('click', 'li', function (e) {
                var _item = othis.editor[othis.aceActive];
            ...
            // 清除搜索内容事件
            $('.menu-files .menu-container .fa').click(function () {
                $('.menu-files .menu-input').val('').next().hide();
                othis.searchRelevance();
            });
            // 顶部状态栏
            $('.ace_header>span').click(function (e) {
                var type = $(this).attr('class'), _item = othis.editor[othis.aceActive];
                if (othis.aceActive == '' && type != 'helps') {
                    return false;
                }
                switch (type) {
                    ...
                    case 'themes':
                        ...
                        for (var i = 0; i < othis.aceConfig.themeList.length; i++) {
                    ...
                    case 'setUp':
                        ...
                        $('.menu-setUp .editor_menu li').each(function (index, el) {
                            var _type = othis.aceConfig.aceEditor[$(el).attr('data-type')];
            ...
            $('.ace_catalogue_list').on('click', '.has-children .file_fold', function (e) {
                var _layers = $(this).attr('data-layer'), _type = $(this).find('data-type'),
                ...
                if ($(this).attr('data-file') == 'Dir') {
                    if (_menu.hasClass('glyphicon-menu-right')) {
                        ...
                        $(this).next().show();
                        if ($(this).next().find('li').length == 0) othis.reader_file_dir_menu({
                            el: $(this).next(),
                            path: _path,
                            group: _group + 1
                        });
            ...
            $('.ace_dir_tools').on('click', '.upper_level', function () {
                ...
                othis.reader_file_dir_menu({path: _paths, is_empty: true});
            ...
            $('.ace_dir_tools').on('click', '.refresh_dir', function (e) {
                ...
                othis.reader_file_dir_menu(othis.refreshConfig, function () {
                    layer.msg('刷新成功', {icon: 1});
                });
            });
            // 搜索内容 (文件目录主菜单)
            $('.ace_dir_tools').on('click', '.search_file', function (e) {
                if ($(this).parent().find('.search_input_view').length == 0) {
                    ...
                    othis.reader_file_dir_menu(othis.refreshConfig);
                }
            });

            // 搜索文件内容
            $('.ace_dir_tools').on('click', '.search_input_view button', function (e) {
                var path = othis.menuPath,
                    search = $('#search_input_val').val();
                othis.reader_file_dir_menu({
                    el: $('.cd-accordion-menu')[0],
                    path: path,
                    group: 1,
                    search: search,
            ...
            $('.ace_dir_tools').on('click', '.folder_down_up li', function (e) {
                ...
                switch (_type) {
                    case 2:
                        othis.newly_file_type_dom(element, group, 0);
                        break;
                    case 3:
                        othis.newly_file_type_dom(element, group, 1);
                        break;
                }
                othis.refresh_config = {
                    el: $('.cd-accordion-menu')[0],
            ...
            $('.ace_catalogue_list').on('mousedown', '.has-children .file_fold', function (e) {
                var x = e.clientX, y = e.clientY, _left = $('.aceEditors')[0].offsetLeft,
                ...
                if (e.which === 3) {
                    ...
                    othis.refresh_config = {
                        ...
                        path: othis.get_file_dir($(this).parent().attr('data-menu-path'), 1),
            ...
            $('.ace_catalogue_list').on('click', '.has-children .edit_file_group .glyphicon-ok', function () {
                ...
                switch (_type) {
                    case 0: //新建文件夹
                        othis.event_create_dir({path: _path + '/' + _file_or_dir});
                        break;
                    case 1: //新建文件
                        othis.event_create_file({path: _path + '/' + _file_or_dir});
                        break;
                    case 2: //重命名
                        othis.event_rename_currency({
                            sfile: _path,
                            dfile: othis.get_file_dir(_path, 1) + '/' + _file_or_dir
                        });
                        break;
                }
            });
            ...
            $('.ace_catalogue_list').on('keyup', '.has-children .edit_file_group input', function (e) {
                ...
                if (_type == 1 && $(this).val().indexOf('.')) $(this).prev().removeAttr('class').addClass(othis.get_file_suffix($(this).val()) + '-icon');
    ...
    var aceEditor = {
        ...
        eventEditor: function () {
            ...
            $('.ace_catalogue_list').on('click', '.has-children .file_fold', function (e) {
                ...
                if ($(this).attr('data-file') == 'Dir') {
                    if (_menu.hasClass('glyphicon-menu-right')) {
                        _menu.removeClass('glyphicon-menu-right').addClass('glyphicon-menu-down');
                        $(this).next().show();
                        if ($(this).next().find('li').length == 0) _this.readerFileDirMenu({
                            el: $(this).next(),
                            path: _path,
                            group: _group + 1
                        });
            ...
            $('.ace_dir_tools').on('click', '.upper_level', function () {
                var _paths = $(this).attr('data-menu-path');
                _this.readerFileDirMenu({path: _paths, is_empty: true});
                $('.ace_catalogue_title').html('目录：' + _paths).attr('title', _paths);
            });
            // 新建文件（文件目录主菜单）
            $('.ace_dir_tools').on('click', '.new_folder', function (e) {
            ...
            $('.ace_dir_tools').on('click', '.refresh_dir', function (e) {
                ...
                _this.readerFileDirMenu(_this.refreshConfig, function () {
                    layer.msg('刷新成功', {icon: 1});
                });
            });
            // 搜索内容 (文件目录主菜单)
            $('.ace_dir_tools').on('click', '.search_file', function (e) {
                if ($(this).parent().find('.search_input_view').length == 0) {
                    ...
                    _this.readerFileDirMenu(_this.refreshConfig);
                }
            });

            // 搜索文件内容
            $('.ace_dir_tools').on('click', '.search_input_view button', function (e) {
                var path = _this.menuPath,
                    search = $('#search_input_val').val();
                _this.readerFileDirMenu({
                    el: $('.cd-accordion-menu')[0],
                    path: path,
                    group: 1,
                    search: search,
            ...
            $('.ace_dir_tools').on('click', '.folder_down_up li', function (e) {
                ...
                switch (_type) {
                    case 2:
                        _this.newFileTypeDom(element, group, 0);
                        break;
                    case 3:
                        _this.newFileTypeDom(element, group, 1);
                        break;
                }
                _this.refresh_config = {
                    el: $('.cd-accordion-menu')[0],
            ...
            $('.ace_catalogue_list').on('mousedown', '.has-children .file_fold', function (e) {
                ...
                if (e.which === 3) {
                    ...
                    _this.refresh_config = {
                        el: $(this).parent().parent()[0],
                        path: _this.getFileDir($(this).parent().attr('data-menu-path'), 1),
                        group: parseInt($(this).attr('data-group')),
                        is_empty: true
                    }
                }
            ...
            $('.ace_catalogue_list').on('click', '.has-children .edit_file_group .glyphicon-ok', function () {
                ...
                switch (_type) {
                    case 0: //新建文件夹
                        _this.eventCreateDir({path: _path + '/' + _file_or_dir});
                        break;
                    case 1: //新建文件
                        _this.eventCreateFile({path: _path + '/' + _file_or_dir});
                        break;
                    case 2: //重命名
                        _this.eventRenameCurrency({
                            sfile: _path,
                            dfile: _this.getFileDir(_path, 1) + '/' + _file_or_dir
                        });
                        break;
                }
            });
            ...
            $('.ace_catalogue_list').on('keyup', '.has-children .edit_file_group input', function (e) {
                ...
                if (_type == 1 && $(this).val().indexOf('.')) $(this).prev().removeAttr('class').addClass(_this.getFileSuffix($(this).val()) + '-icon');
                $(this).attr('data-type', 1);
                $(this).css('border', '1px solid #528bff');
                layer.closeAll('tips');
                if (e.keyCode === 13) $(this).next().click();
        ...
        newly_file_type: function (that) {
            ...
            switch (_type) {
                ...
                case 2: //新建文件
                case 3:
                    if (this.get_file_dir(_path, 1) != this.menuPath) { //判断当前文件上级是否为显示根目录
                        this.reader_file_dir_menu({el: _active, path: _path, group: _group + 1}, function (res) {
                            _this.newFileTypeDom(_active, _group, _type == 2 ? 0 : 1);
                        });
                    } else {
                        _this.newFileTypeDom(_active, _group, _type == 2 ? 0 : 1);
                    }
        ...
        // 恢复历史文件事件
        event_recover_file: function (that) {
        ...
        reader_file_dir_menu: function (obj, callback) {
            ...
            this.get_file_dir_list({p: obj.p, path: obj.path, search: obj.search, all: obj.all}, function (res) {
                ...
                for (var j = 0; j < _files.length; j++) {
                    ...
                    _files_dom += '<li class="has-children" title="' + (obj.path + '/' + _data[0]) + '" data-menu-path="' + (obj.path + '/' + _data[0]) + '" data-size="' + (_data[1]) + '" data-suffix="' + _this.getFileSuffix(_data[0]) + '">\
					<div class="file_fold  group_' + obj.group + '" data-group="' + obj.group + '" data-file="Files">\
						<span class="file_title"><i class="' + _this.getFileSuffix(_data[0]) + '-icon"></i><span>' + _data[0] + '</span></span>\
					</div>\
				</li>';
                }
                if (res.PATH !== '/' && obj['group'] === 1) {
                    $('.upper_level').attr('data-menu-path', _this.getFileDir(res.PATH, 1));
        ...
        get_file_suffix: function (fileName) {
            ...
            for (var name in this.aceConfig.supportedModes) {
                var data = this.aceConfig.supportedModes[name], suffixs = data[0].split('|'),
        ...
        setEditorView: function () {
            ...
            var autoAceHeight = setInterval(function () {
                ...
                var ace_conter_menu = $('.ace_container_menu').height();
                var ace_conter_toolbar = $('.ace_container_toolbar').height();
        ...
        openHistoryEditorView: function (obj, callback) {
            ...
            var _this = this, path = obj.filename, paths = path.split('/'), _fileName = paths[paths.length - 1],
        ...
        openEditorView: function (path, callback) {
            ...
            // 文件类型（type，列如：JavaScript） 、文件模型（mode，列如：text）、文件标识（id,列如：x8AmsnYn）、文件编号（index,列如：0）、文件路径 (path，列如：/www/root/)
            var _this = this, paths = path.split('/'), _fileName = paths[paths.length - 1],
                _fileType = this.getFileType(_fileName), _type = _fileType.name, _mode = _fileType.mode,
                _id = bt.get_random(8), _index = this.editorLength;
            _this.isFileOpen(path, function (is_state) {
                if (is_state) {
                    $('.ace_container_menu').find('[title="' + path + '"]').click();
                } else {
        ...
        // 递归保存文件
        saveAllFileBody: function (arr, num, callback) {
            var _this = this;
            ...
            aceEditor.saveFileBody({
                path: arr[num].path,
                data: arr[num].data,
                encoding: arr[num].encoding
            }, function () {
                num = num + 1;
                aceEditor.saveAllFileBody(arr, num, callabck);
            });
        }
    }
...
//会话失效时自动跳转到登录页面
    ...
    function ChangePath(d) {
        ...
        var c = layer.open({
            ...
            < span class = 'glyphicon glyphicon-share-alt' > < /span> " + lan.public.return + "
    </button>
        ...
        <div class='path-con'>
            <div class='path-con-left'>
                <dl>
                    <dt id='changecomlist' onclick='BackMyComputer()'>" + lan.bt.comp + "</dt>
                </dl>
            </div>
                ...
                <div class='file-list divtable'>
                    <table class='table table-hover' style='border:0 none'>
                        ...
                        <tr class='file-list-head'>
                            <th width='40%'>" + lan.bt.filename + "</th>
                            <th width='20%'>" + lan.bt.etime + "</th>
                            <th width='10%'>" + lan.bt.access + "</th>
                            <th width='10%'>" + lan.bt.own + "</th>
                            <th width='10%'></th>
                        </tr>
                        </thead>
        ...
        <div class='getfile-btn' style='margin-top:0'>
            <button type='button' class='btn btn-default btn-sm pull-left' onclick='CreateFolder()'>" + lan.bt.adddir +
                "
            </button>
            <button type='button' class='btn btn-danger btn-sm mr5' onclick=\"layer.close(getCookie('ChangePath'))\">" +
                lan.public.close + "
        </button>
        <button type='button' class='btn btn-success btn-sm' onclick='GetfilePath()'>" + lan.bt.path_ok + "</button>
    </div>
        "
    })
        ;
        ...
        $.post("/files?action=GetDir", c, function (h) {
            if (h.DISK != undefined) {
                for (var f = 0; f < h.DISK.length; f++) {
                    a += "
                        < dd
                    onclick =\"GetDiskList('" + h.DISK[f].path + "')\"><span class='glyphicon glyphicon-hdd'></span>&nbsp;<span>" + h.DISK[f].path + "</span></div></dd>"
                }
            ...
            for (var f = 0; f < h.DIR.length; f++) {
                ...
                d += "
                    < tr >
                    < td
                onclick =\"GetDiskList('" + h.PATH +
                "/" + g[0] + "')\" title='" + g[0] + "'><span class='glyphicon glyphicon-folder-open'></span>" + e + "</td>
                < td > " + getLocalTime(g[2]) + " < /td>
                <td>" + g[3] + "</td>
                <td>" + g[4] + "</td>
                <td><span class='delfile-btn' onclick=\"NewDelFile('" + h.PATH + "/" + g[0] + "')\">X</span>
            </td>
            </tr>
                "
            }
            if (h.FILES != null && h.FILES != "") {
                for (var f = 0; f < h.FILES.length; f++) {
                    ...
                    = 'glyphicon glyphicon-file' > < /span><span>" + e + "</s
                    pan > < /td>
                    <td>" + getLocalTime(g[2]) + "</td>
                    <td>" + g[3] + "</td>
                    <td>" + g[4] + "</td>
    ...
    function CreateFolder() {
        ...
        <input id='newFolderName' class='newFolderName' type='text' value=''></td>
        <td colspan='3'>
            <button id='nameOk' type='button' class='btn btn-success btn-sm'>" + lan.public.ok + "</button>
            &nbsp;&nbsp;
            <button id='nameNOk' type='button' class='btn btn-default btn-sm'>
                " + lan.public.cancel + "
            </button>
        </td>
    </tr>
    ...
    function OnlineEditFile(k, f) {
        ...
        $.post("/files?action=GetFileBody", "path=" + encodeURIComponent(f), function (s) {
            ...
            var r = layer.open({
                ...
                content: '
                    < form
                class = "bt-form pd20 pb70" >
                < div
                class = "line" > < p
                style = "color:red;margin-bottom:10px" > ' + lan.bt.edit_ps + '
                < select
                class = "bt-input-text"
                name = "encoding"
                style = "width: 74px;position: absolute;top: 31px;right: 19px;height: 22px;z-index: 9999;border-radius: 0;" > '
                + n + '</select></p>
                < textarea
                class = "mCustomScrollbar bt-input-text"
                id = "textBody"
                style = "width:100%;margin:0 auto;line-height: 1.8;position: relative;top: 10px;"
                value = "" / >
                < /div>
            <div class="bt-form-submit-btn" style="position:absolute; bottom:0; width:100%">
                ...
                <button id="OnlineEditFileBtn" type="button" class="btn btn-success btn-sm">' + lan.public.save + '
                </button>
            </div>
        </form>
            '
    ...
    function GetPHPStatus(a) {
        ...
        $.post("/ajax?action=GetPHPStatus", "version=" + a, function (b) {
            layer.open({
                ...
                < tr >
                < th > " + lan.bt.php_pool + " < /th>
            <td>" + b.pool + "</td>
        </tr>
            <tr>
                <th>" + lan.bt.php_manager + "</th>
                <td>" + ((b["process manager"] == "dynamic") ? lan.bt.dynamic : lan.bt.static) + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_start + "</th>
                <td>" + b["start time"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_accepted + "</th>
                <td>" + b["accepted conn"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_queue + "</th>
                <td>" + b["listen queue"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_max_queue + "</th>
                <td>" + b["max listen queue"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_len_queue + "</th>
                <td>" + b["listen queue len"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_idle + "</th>
                <td>" + b["idle processes"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_active + "</th>
                <td>" + b["active processes"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_total + "</th>
                <td>" + b["total processes"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_max_active + "</th>
                <td>" + b["max active processes"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_max_children + "</th>
                <td>" + b["max children reached"] + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.php_slow + "</th>
                <td>" + b["slow requests"] + "</td>
            </tr>
        </table>
        </div>
            "
    ...
    function GetNginxStatus() {
        $.post("/ajax?action=GetNginxStatus", "", function (a) {
            layer.open({
                ...
                < table class = 'table table-hover table-bordered' >
                < tr >
                < th > " + lan.bt.nginx_active + " < /th>
            <td>" + a.active + "</td>
        </tr>
            <tr>
                <th>" + lan.bt.nginx_accepts + "</th>
                <td>" + a.accepts + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.nginx_handled + "</th>
                <td>" + a.handled + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.nginx_requests + "</th>
                <td>" + a.requests + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.nginx_reading + "</th>
                <td>" + a.Reading + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.nginx_writing + "</th>
                <td>" + a.Writing + "</td>
            </tr>
            <tr>
                <th>" + lan.bt.nginx_waiting + "</th>
                <td>" + a.Waiting + "</td>
            </tr>
        </table>
        </div>
    ...
    function SafeMessage(j, h, g, f) {
        ...
        var mess = layer.open({
            ...
            shadeClose: true,
            content: "
                < div class = 'bt-form webDelete pd20 pb70' > < p > " + h + " < /p>" + f + "
        <div class='vcode'>" + lan.bt.cal_msg +
            "<span class='text'>" + sumtext + "</span>=<input type='number' id='vcodeResult' value=''></div>
        <div class='bt-form-submit-btn'>
            <button type='button' class='btn btn-danger btn-sm bt-cancel'>" + lan.public.cancel + "</button>
            <button type='button' id='toSubmit' class='btn btn-success btn-sm'>" + lan.public.ok + "</button>
        </div>
    </div>
        "
    })
...
//检查弱口令
        ...
        layer.open({
            ...
            < div class = 'line' > < span class = 'tname' > " + lan.public.pass + " < /span>
        <div class='info-r'>
            <input class='bt-input-text' type='text' name='password1' id='p1' value='' placeholder='" + lan.bt.pass_new_title + "' style='width:100%'/>
        </div>
    </div>
        <div class='line'><span class='tname'>" + lan.bt.pass_re + "</span>
            <div class='info-r'>
                <input class='bt-input-text' type='text' name='password2' id='p2' value='' placeholder='" + lan.bt.pass_re_title + "' style='width:100%'/>
            </div>
        </div>
        <div class='bt-form-submit-btn'>
            <span style='float: left;' title='" + lan.bt.pass_rep + "' class='btn btn-default btn-sm' onclick='randPwd(10)'>" + lan.bt.pass_rep_btn + "</span>
            <button type='button' class='btn btn-danger btn-sm' onclick=\"layer.closeAll()\">" + lan.public.close + "
        </button>
        <button type='button' class='btn btn-success btn-sm' onclick=\
        "setPassword(1)\">" + lan.public.edit + "</button>
        < /div>
    </div>
        "
    })
    ...
    function setUserName(a) {
        ...
        layer.open({
            ...
            content: "
                < div class = 'bt-form pd20 pb70' >
            < div class = 'line' > < span class = 'tname' > " + lan.bt.user + " < /span>
        <div class='info-r'>
            <input class='bt-input-text' type='text' name='password1' id='p1' value='' placeholder='" + lan.bt.user_new + "' style='width:100%'/>
        </div>
    </div>
        <div class='line'><span class='tname'>" + lan.bt.pass_re + "</span>
            <div class='info-r'>
        ...
        <button type='button' class='btn btn-success btn-sm' onclick=\
        "setUserName(1)\">" + lan.public.edit + "</button>
        < /div>
    </div>
        "
    })
        ...
        $.post("/ajax?action=GetSoftList", "", function (l) {
            ...
            for (var h = 0; h < l.length; h++) {
                ...
                var m = "
                    < input
                id = 'data_" + l[h].name + "'
                data - info = '" + l[h].name + " " + l[h].versions[0].version + "'
                type = 'checkbox'
                checked > ";
                for (var b = 0; b < l[h].versions.length; b++) {
                    ...
                    if ((l[h].name == "PHP" && (l[h].versions[b].version == "5.4" || l[h].versions[b].version == "54")) || (l[h].name == "MySQL" && l[h].versions[b].version == "5.5") || (l[h].name == "phpMyAdmin" && l[h].versions[b].version == "4.4")) {
                        ...
                        m = "
                            < input
                        id = 'data_" + l[h].name + "'
                        data - info = '" + l[h].name + " " + l[h].versions[b].version + "'
                        type = 'checkbox'
                        checked > "
                    }
                    o += "
                        < option
                    value = '" + l[h].versions[b].version + "'
                    " + d + " > " + l[h].name + "
                    " + l[h].versions[b].version + " < /option>"
                }
                var f = "
                    < li >
                    < span
                ...
                = 'ico' > < img
                src = '/static/img/" + l[h].name.toLowerCase() + ".png' > < /span><span class='name'><select id='select_" + l[h].name + "' class='sl-s-info'>" + o + "</s
                elect > < /span><span class='pull-right'>" + m + "</s
                pan >
                < /li>";
                if (l[h].name == "Nginx") {
                    c = f
            ...
            var k = layer.open({
                ...
                < div class = 'important-title' > < p >
                < span class = 'glyphicon glyphicon-alert' style = 'color: #f39c12; margin-right: 10px;' > < /span>" +
                lan.bt.install_ps + " <a href='javascript:jump()' style='color:#20a53a'>" + lan.bt.install_s + "</a> " +
                    lan.bt.install_s1 + "</p></div>
                    < div class = 'rec-box' > < h3 > " + lan.bt.install_lnmp + " < /h3>
            <div class='rec-box-con'>
                <ul class='rec-list'>" + c + "</ul>
                <p class='fangshi'>" + lan.bt.install_type +
                    "：<label data-title='" + lan.bt.install_rpm_title + "' style='margin-right:0'>" + lan.bt.install_rpm
                        + "<input type='checkbox' checked></label><label data-title='" + lan.bt.install_src_title + "'>"
                        + lan.bt.install_src + "<input type='checkbox'></label></p>
                <div class='onekey'>" + lan.bt.install_key + "</div>
            </div>
        </div>
            <div class='rec-box' style='margin-left:16px'><h3>LAMP</h3>
                <div class='rec-box-con'>
                    <ul class='rec-list'>" + g + "</ul>
                    <p class='fangshi'>" + lan.bt.install_type +
                        "：<label data-title='" + lan.bt.install_rpm_title + "' style='margin-right:0'>" +
                            lan.bt.install_rpm +
                            "<input type='checkbox' checked></label><label data-title='" + lan.bt.install_src_title + "'>"
                            + lan.bt.install_src + "<input type='checkbox'></label></p>
...
//        for(var i=0; i
                ...
                {
//            con +='
                    <h3 class="mypcip mypcipnew" style="opacity:.6" data-url="'+rdata[i].url+'" data-user="'+rdata[i].username+'" data-pw="'+rdata[i].password+'">
                        <span class="f14 cw">'+rdata[i].title+'</span><em class="btedit" onclick="bindBTPanel(0,\'c\',\''+rdata[i].title+'\',\''+rdata[i].id+'\',\''+rdata[i].url+'\',\''+rdata[i].username+'\',\''+rdata[i].password+'\')"></em>
                    </h3>
                    '
                    //        }
                    //        $("#newbtpc").html(con);
                    ...
                    <iframe name="btpfrom" src=""></iframe>
                </div>
                    ';
                    ...
                    onclick =\"bindBTPanel(1,'b')\">" + lan.public.add + "</button>";
//    }
//    else{
//        titleName = lan.bt.panel_edit+ip;
//        btn = "
                    <button type='button' class='btn btn-default btn-sm' onclick=\
                    "bindBTPaneldel('" + btid + "')\">" + lan.public.del + "</button>
                    < button
                    type = 'button'
                    ...
                    = 'btn btn-success btn-sm'
                    onclick =\"bindBTPanel(1,'c','" + ip + "','" + btid + "')\" style='margin-left:7px'>
                    "+lan.public.edit+"
                    < /button>";
//    }
...
//        content: "
                    <div class='bt-form pd20 pb70'>\
                        ...
                        <div class='line'><span class='tname'>"+lan.bt.panel_address+"</span>\
                            //
                            <div class='info-r'>
                                <input class='bt-input-text' type='text' name='btaddress' id='btaddress' value='"+url+"' placeholder='"+lan.bt.panel_address+"' style='width:100%'/>
                            </div>
                            \
                            //
                        </div>
                        ...
                        <div class='line'><span class='tname'>"+lan.bt.panel_user+"</span>\
                            //
                            <div class='info-r'>
                                <input class='bt-input-text' type='text' name='btuser' id='btuser' value='"+user+"' placeholder='"+lan.bt.panel_user+"' style='width:100%'/>
                            </div>
                            \
                            //
                        </div>
                        ...
                        <div class='line'><span class='tname'>"+lan.bt.panel_pass+"</span>\
                            //
                            <div class='info-r'>
                                <input class='bt-input-text' type='password' name='btpassword' id='btpassword' value='"+pw+"' placeholder='"+lan.bt.panel_pass+"' style='width:100%'/>
                            </div>
                            \
                            //
                        </div>
                        ...
                        <div class='line'><span class='tname'>"+lan.bt.panel_ps+"</span>\
                            //
                            <div class='info-r'>
                                <input class='bt-input-text' type='text' name='bttitle' id='bttitle' value='"+ip+"' placeholder='"+lan.bt.panel_ps+"' style='width:100%'/>
                            </div>
                            \
                            //
                        </div>
                        ...
                        <div class='line'>
                            <ul class='help-info-text c7'>
                                <li>"+lan.bt.panel_ps_1+"</li>
                                <li>"+lan.bt.panel_ps_2+"</li>
                                <li>"+lan.bt.panel_ps_3+"</li>
                            </ul>
                        </div>
                        \
                        //
...
<div class="bt-progress">
                            ...
                            = "bt-progress-bar"
                            style = "width:'+ speed.progress + '%" > < span

                            class

                            = "bt-progress-text" > ' + speed.progress + ' %
                        </span>
                        </div>
                        </div>\
...
//消息盒子
    ...
    <div class="bt-w-main">\
        <div class="bt-w-menu">\
            <p class="bgw" id="taskList" onclick="tasklist()">'+ lan.bt.task_list + '(<span class="task_count">0</span>)
            </p>\
    ...
    \
                    ...
                    function reader_message_list(page) {
                        get_message_data(page, function (res) {
                            ...
                            for (var i = 0; i < res.data.length; i++) {
                                ...
                                = "titlename c3" > ' + item.name + ' < /span>
                                <span class="rs-status">【' + lan.bt.task_ok + '】<span><span class="rs-time">' + lan.bt.time + (item.end - item.start) + lan.bt.s + '</span>
        </div>
    </td>
                                <td class="text-right c3">' + item.addtime + '</td>
                            </tr>
                                '
                            }
                            var con = '
        ...
        <thead>
        <tr>
            <th>'+ lan.bt.task_name + '</th>
            <th class="text-right">' + lan.bt.task_time + '</th>
        </tr>
        </thead>
        \
                        ...
                        get_realtime_tasks(function (res) {
                            ...
                            if (typeof res.task === "undefined") {
                                ...
                                if (command_install_list.find('li').length) {
                                    ...
                                    if (task[0].status !== '0' && !command_install_list.find('pre').length) command_install_list.find('li:eq(0)').append('
                                        < pre

                                    class

                                    =\'cmd command_output_pre\'>' + shell + '</pre>'
                                )
                                    messageBoxWssock.el = command_install_list.find('pre');
                                } else {
                                    for (var i = 0; i < task.length; i++) {
                                        var item = task[i], task_html = '', del_task = '
                                            < a
                                        style = "color:green"
                                        onclick = "RemoveTask(' + item.id + ')"
                                        href = "javascript:;" > ' + lan.public.del +
                                        '</a>', loading_img = "<img src='" + loading + "'/>";
                                        if (item.status === '-1' && item.type === 'download') {
                                            ...
                                            = 'line-progress'
                                            style = 'width:" + message.pre + "%' > < /div>
                                            <span class='titlename'>" + item.name + "<a style='margin-left:130px;'>" + (ToSize(message.used) + "/" + ToSize(message.total)) + "</a></span>
                                            <span class='com-progress'>" + message.pre + "%</span>
                                            <span class='state'>" + lan.bt.task_downloading + " " + loading_img + " | " + del_task + "</span>
                                            ";
                                        } else {
...
<div class="term-box" style="background-color:#000" id="term"></div>
                                ...
                                cancel: function (index, lay) {
                                    bt.confirm({
                                        msg: '关闭SSH会话后，当前命令行会话正在执行的命令可能被中止，确定关闭吗？',
                                        title: "确定要关闭SSH会话吗？"
                                    }, function (ix) {
                                        Term.term.dispose();
                                        layer.close(index);
                        ...
                        localhost_login_form: function () {
                            ...
                            = "localhost-form-view bt-form-2x" >
                                < div

                            class

                            = "localhost-form-title" > < i

                            class

                            = "localhost-form_tip" > < /i><span style="vertical-align: middle;">无法自动认证，请填写本地服务器的登录信息!</s
                            pan >
                            < /div>
        \
        ...
        <div class="line">\
            ...
            <div class="info-r ">\
                <div class="btn-group">\
                    <button type="button" tabindex="-1" class="btn btn-sm auth_type_checkbox btn-success" data-ctype="0">
                        密码验证
                    </button>
                    \
                    <button type="button" tabindex="-1" class="btn btn-sm auth_type_checkbox btn-default data-ctype=" 1
                    ">私钥验证</button>\
            </div>
            \
        </div>
        ...
        <div class="line c_pkey_view hidden">\
            ...
            \
                    ...
                    acme = {
                        speed_msg: "
                            < pre
                        style = 'margin-bottom: 0px;height:250px;text-align: left;background-color: #000;color: #fff;white-space: pre-wrap;'
                        id = 'create_lst' > [MSG] < /pre>",
                        loadT: null,
//获取订单列表
...
//取证书基本信息
                        get_cert_init: function (pem_file, siteName, callback) {
                            acme.request('get_cert_init_api', {
                                pem_file: pem_file,
                                siteName: siteName
                            }, function (cert_init) {
                                callback(cert_init);
...
// console.log("websocket连接异常！")
                                    ...
                                    this.bws.addEventListener('message', function (ev) {
                                        that.on_message(ev)
                                    });
                                    this.bws.addEventListener('close', function (ev) {
                                        that.on_close(ev)
                                    });
                                    this.bws.addEventListener('error', function (ev) {
                                        that.on_error(ev)
                                    });
                                    this.bws.addEventListener('open', function (ev) {
                                        that.on_open(ev)
                                    });
...
// 显示消息
                                            if (_result.type == 1 && _result.status == undefined) {
                                                return true;
                                            } else if (_result.type == 0 && _result.status == undefined) {
                                                return true;
                                            }
...
//直接进行已读未读
                                            ...
                                            } else if (_result.content == "close" || _result.type == 6 || _result.type == 2) {
                                                _this.unread_messages.push(result);
                                            }
// 没有打开聊天窗口，更新红色气泡提示
// 检查气泡是否显示
                                            ...
                                            try {
                                                ...
                                                for (var a = 0; a < _this.unread_messages.length; a++) {
                                                    ...
                                                    if (_message.status == 0 && _message.receiver == _this.bt_user && _message.from_client != true) {
                                                        num = num + 1;
                                                    } else if (_message.type == 5 || _message.type == 6 || _message.type == 2) {
...
//关闭连接
                        ...
                        init: function () {
                            ...
                            = "debugs" > < span > 在线 < /br>客服</s
                            pan > < /div><span class="badge chat-number">0条未读</s
                            pan > '));
                            body.append($('
                                < div
                            ...
                            document.write("
                                < script
                            language = javascript
                            src = '/static/js/html2canvas.min.js' > <\/script>");
                            $('#feedback-box').hide();
                            $('.chat-number').hide();
                            $('.debugs').hide();
                            //获取用户信息
                            ...
                            $('.debugs').on("click", function (e) {
                                ...
                                if (_this.bt_user != undefined && _this.bt_uid != undefined) {
                                    ...
                                    if (_this.interface_flag == "feedback") { // 工单提交界面
                                        ...
                                        e.stopPropagation();
                                    } else if (_this.interface_flag == "history") {
                                        $("#feedback-box").fadeToggle(200);
                                        _this.history_box(flag, "history");
                                    } else if (_this.interface_flag == "chat") {
                                        $("#feedback-box").fadeToggle(200);
                                        $('.debugs').empty();
                        ...
                        feedback_chat: function () {
                            var _this = this;
                            _this.chat_show(JSON.stringify({
                                "id": "feedback",
                                "workorder": "feedback",
                                "content": "您好，欢迎使用堡塔在线客服系统，我们的工作时间为：周一到周六 上午10:00-下午18:00",
                                "receiver": "admin",
                                "type": 0
                            }));
                            if (_this.bt_allow == false) {
                                _this.chat_show(JSON.stringify({
                                    "id": "EE",
                                    "workorder": "EE",
                                    "title": "当前未购买企业版,请开通企业版",
                                    "content": "开通企业版优势:</br>1、可以使用在线客服提交工单协助处理面板问题。</br>2、可以免费使用专业版及企业版的数十款插件。",
                                    "btn": true,
                                    "btn_content": "立即开通企业版",
                                    "receiver": "admin",
                                    "type": 0
                                ...
                                _this.chat_show(JSON.stringify({
                                    "id": "feedback",
                                    "workorder": "feedback",
                                    "content": "请详细描述您遇到的问题，并附带截图，以便我们能更好的跟进，谢谢。",
                                    "receiver": "admin",
                                    "type": 0
                                }));
                            }
				...
				<input type="button" class="btn btn-sm btn-danger button-size stop-chat"  style="outline: none;" value="退出会话"></button>\
                        ...
                        send_message: function (content, type, id) {
                            ...
                            if (_this.interface_flag == "chat") {
                                ...
                                if (_this.bws) {
                                    try {
                                        type == 3 ? _this.send_server(_this, JSON.stringify({
                                            "id": id,
                                            "type": 3,
                                            "content": content,
                                            "workorder": workorder
                                        })) : _this.send_server(_this, JSON.stringify({
                                            "id": snow_id,
                                            "type": type,
                                            "content": content,
                                            "workorder": workorder,
                                            "from_client": true,
                                            "sender": _this.bt_user
                                        }));
                                    } catch (err) {
                                        // console.log("消息发送失败")
                                    }
					...
					<div class="feedback-bottom">\
						...
						<button type="button" class="btn btn-sm btn-danger feedback-button-group feedback-close" style="right: 97px;">取消</button>\
                            ...
                            $(".feedback-submit").on("click", function (e) {
                                ...
                                var imgf_flag = $("#imgf").find("img").attr("src") ? true : false;
                                imgs_flag = $("#imgs").find("img").attr("src") ? true : false;
                                imgt_flag = $("#imgt").find("img").attr("src") ? true : false;
                                if (_length == 0) {
			...
			<span class = "finish">完成</span>\
                            ...
                            html2canvas(body[0], {useCORS: true, x: 0, y: 0, scale: 1}).then(function (canvas) {
                                ...
                                $('.toolbar span:eq(0)').on('mousedown', function (e) {
                                    e = e || window.event;
                                    var toolbar = $('.toolbar'), toolbar_left = toolbar.offset().left,
                                        toolbar_top = toolbar.offset().top;
                                    $(document).on('mousemove', function (ev) {
                                        var win = $(window)[0], win_height = win.innerHeight,
                                            win_width = win.innerWidth,
                                            offsetX = ev.clientX - e.clientX, offsetY = ev.clientY - e.clientY,
                                            config = {right: 'inline', bottom: 'inline'};
                                ...
                                $('.mask_select_view').on('mousedown', function (ev) {
                                    var x = ev.clientX, y = ev.clientY, width = 0, height = 0,
                                        status = $('.toolbar span.active').index() === 1;
                                    ...
                                    $(this).on('mousemove', function (ev) {
                                        var move_x = ev.clientX, move_y = ev.clientY,
                                            left = (x > move_x ? move_x : null || x < move_x ? x : null),
                                            top = (y > move_y ? move_y : null || y < move_y ? y : null);
                            ...
                            $("body").css("overflow", "hidden").append(mask),
                                ...
                                $(".preview_images_mask #preview_images").on("mousedown", (function (e) {
                                        ...
                                        $(".preview_images_mask").on("mousemove", (function (e) {
                                                var offsetX = (e = e || window.event).clientX - preview.left - diffX,
...
// 用户绑定
          ...
          <div class="bt-form text-center">\
              ...
              <p class="text-right"><a class="btlink" href="https://www.bt.cn/register.html" target="_blank">未有账号，免费注册</a></p>\
                                ...
                                success: function () {
                                    ...
                                    $('.bind_ps').hover(function () {
                                        ...
                                        time = setTimeout(function () {
                                            layer.tips('宝塔面板许多功能都依赖于官网，绑定仅限于为您提供更好的面板服务体验，不涉及您服务器任何敏感信息，请放心使用。', _that, {
                                                tips: [1, '#20a53a'],
                                                time: 0
                                            })
                                        }, 500);
                                ...
                                cancel: function () {
                                    if (!type) {
                                        layer.alert('<ul class="help-info-text" style="margin-top:0;">\
          <li>为了您能更好的体验面板功能，请先绑定宝塔账号；</li>\
          <li>单个宝塔帐号支持绑定多台服务器；</li>\
