    ...
    class ClassTree {
        ...
        builder(box, fields, parentIndex) {
            for (let i = 0; i < fields.length; i++) {
                ...
                fields[i].index = this.index;
                this.data[fields[i].index] = fields[i];
                this.names[fields[i].name] = fields[i].index;
                this.index++;
        ...
        render(elem, rawData) {
            ...
            elem.html(boxHTML);
            this.builder(elem.find('ul.tree'), rawData, -1);
            let othis = this,
                push = {
                ...
                active = {
                    fill: function () {
                        ...
                        if (Array.isArray(items) && items.length > 0) {
                            ...
                            elem.find('ul.tree>li').remove();
                            othis.builder(elem.find('ul.tree'), fields, -1);
                        } else {
                            main.err("请设置栏目名称库");
                            return false;
                        }
            ...
            // 活动事件
            elem.off('click').on('click', 'ul.tree i[data-event]', function (e) {
                let $this = $(this), event = $this.data('event');
                active[event] && active[event].call($this);
                e.stopPropagation();
            });
            elem.on('click', 'ul.tree i.arrow-right', function (e) {
