layui.define(['main'], function (exports) {
    ...
    class Editor {
        ...
        getFileType(filename) {
            ...
            for (let name in this.aceConfig['supportedModes']) {
                if (this.aceConfig['supportedModes'].hasOwnProperty(name)) {
        ...
        newFileFolderDOM(action) {
            ...
            HTML += okAndCancelHTML + '</span></div></li>';
        ...
        render(filename) {
            let othis = this,
                functions = {
                    ok: function () {
                        let name = this.parent().prev('input').val().trim(),
                            action = parseInt(this.closest('.file-fold[data-edit]').attr('data-edit')) || 1;
                        if (name === '') {
                            this.parent().prev().css('border', '1px solid #f34a4a');
                            layer.tips(action === 1 ? '目录名称不能为空' : (action === 2 ? '文件名称不能为空' : '新名称不能为空'), this.parent().prev(), {
                                tips: [1, '#f34a4a'],
                                time: 0
                            });
                            return false;
                        }
                        if (this.parent().prev().data('type') === 1) return false;
                        let active = othis.getActiveConfig();
                        if (action === 3 && active.elem.find('.file-title span').text() === name) {
                            this.parent().prev().css('border', '1px solid #f34a4a');
                            layer.tips('您并没有修改名称!', this.parent().prev(), {tips: [1, '#f34a4a'], time: 0});
                            return false;
                        }
                        if (action === 3) {
                            othis.renderFiles({
                    ...
                    rename: function () {
                        ...
                        active.elem.prepend('<span class="file-input"><i class="'
                            ...
                            + '" data-event="edit">' + okAndCancelHTML);
                        ...
                        layer.open({
                            ...
                            yes: function (index) {
                                for (let item in othis.editors) {
                                    if (othis.editors.hasOwnProperty(item) && othis.editors[item].fileState !== 0) {
                    ...
                    themes: function () {
                        ...
                        othis.dom.find('.menu-themes ul li').on('click', function () {
                            ...
                            othis.saveAceConfig(othis.aceConfig, function () {
                                for (let item in othis.editors) {
                                    if (othis.editors.hasOwnProperty(item)) {
    ...
    layui.link('/static/file/icons/icons.css');
    layui.link('/static/file/file.css');
