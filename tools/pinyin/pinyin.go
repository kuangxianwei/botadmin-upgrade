...
var reFinalExceptions = regexp.MustCompile("^([jqx])([ūúǔù])$")
var reFinal2Exceptions = regexp.MustCompile("^([jqx])u(\\d?)$")
...
//返回拼音拼接结果
func pinyinJoin(s string, a Args) string {
	var pys []string
	for _, r := range s {
		py := SinglePinyin(r, a)
		if len(py) > 0 {
	...
	return strings.Join(pys, a.Separator)
}

//返回拼音
func Fill(word string, sep ...string) string {
	conf := NewArgs()
	if len(sep) > 0 {
		conf.Separator = sep[0]
	} else {
		conf.Separator = ""
	}
	return pinyinJoin(word, conf)
}

//返回拼音首字母
func First(word string, sep ...string) string {
	conf := NewArgs()
	if len(sep) > 0 {
		conf.Separator = sep[0]
	} else {
