...
// IniConfig implements Cfg to parse ini file.
type IniConfig struct{}

// Parse creates a new Cfg and parses the file configuration from the named file.
func (ini *IniConfig) Parse(name string) (Configure, error) {
	return ini.parseFile(name)
}
func (ini *IniConfig) parseFile(name string) (*IniConfigContainer, error) {
	//配置文件不存在则创建
	data, err := WithCreateReadFile(name)
	if err != nil {
		return nil, err
	}
...
func (ini *IniConfig) ParseData(data []byte) (Configure, error) {
	...
	if err == nil {
		dir = "botadmin-" + currentUser.Username
	}
...
// IniConfigContainer A Cfg represents the ini configuration.
...
func (c *IniConfigContainer) Strings(key string) []string {
	...
	return strings.Split(v, ArrowSeparator)
}
...
func (c *IniConfigContainer) SaveConfigFile(filename string) (err error) {
	...
	// GetPc section or key comments. IsFace #1607
	...
	if dt, ok := c.data[defaultSection]; ok {
		for key, val := range dt {
			if key != " " {
				...
				var line string
				if val == "" {
					line = key + val + lineBreak
				} else {
					line = key + string(bEqual) + val + lineBreak
				}
...
func NewIni(configFile string) (Configure, error) {
	return new(IniConfig).Parse(configFile)
}
func init() {
	register("ini", NewIni)
}
